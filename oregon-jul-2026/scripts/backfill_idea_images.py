...
ROOT = os.path.dirname(HERE)          # trip root (holds the image folders + HTML)
DATA = os.path.join(ROOT, "data")     # manifests / weather / itineraries
...
mpath = os.path.join(DATA, "idea_images_manifest.json")
...
for key, (target, query, must_any, must_not) in BACKFILL.items():
    ...
    for c in cands:
        ...
        try:
            size = download(c["url"], os.path.join(ROOT, rel))
